...
class LoginController: UIViewController {
    ...
    @objc fileprivate func handleRegister(){
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (res, err) in
            ...
            let ref = Database.database().reference(fromURL: "https://chat-bfeb2.firebaseio.com/")
            let userReference = ref.child("users").child(uid)
            let values = ["name": name, "email": email]
            userReference.updateChildValues(values, withCompletionBlock: { (err, ref) in
                if let err = err {
                    print("Failed inserting values into database", err)
                    return
                }
                print("Successfully user register into Firebase ")
            })
        }
        print(123)
    }
    ...
    func setupLoginRegistrationButton(){
        ...
        loginRegistrationButton.topAnchor.constraint(equalTo: inputsContainerView.bottomAnchor, constant: 12).isActive = true
        loginRegistrationButton.widthAnchor.constraint(equalTo: inputsContainerView.widthAnchor).isActive = true
        loginRegistrationButton.heightAnchor.constraint(equalToConstant: 50).isActive = true
    }
